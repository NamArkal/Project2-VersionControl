...
void DFS_r_ltr(GraphProcessing::Sptr<V> pNode, P p)
{
	if (pNode->marked())
		return;
	pNode->mark();
...
template<typename V, typename P>
void DFS_r_ltrbtt(GraphProcessing::Sptr<V> pNode, P p)
{
	if (pNode->marked())
		return;
	pNode->mark();
...
template<typename V, typename P>
void DFS_r_rtl(GraphProcessing::Sptr<V> pNode, P p)
{
	if (pNode->marked())
		return;
	pNode->mark();
...
*/
template<typename V, typename P>
void DFS_s_rtl(GraphProcessing::Sptr<V> pNode, P p)
{
	std::stack<GraphProcessing::Sptr<V>> nodeStack;
	nodeStack.push(pNode);
	while (nodeStack.size() > 0)
	{
		GraphProcessing::Sptr<V> pCurrentNode = nodeStack.top();
		nodeStack.pop();
		pCurrentNode->mark();
		p(pCurrentNode);
...
template<typename V, typename P>
void DFS_s_ltr(GraphProcessing::Sptr<V> pNode, P p)
{
	std::stack<GraphProcessing::Sptr<V>> nodeStack;
	nodeStack.push(pNode);
	while (nodeStack.size() > 0)
	{
		...
		for (size_t i = 0; i < numChildren; ++i)
		{
			GraphProcessing::Sptr<V> pChild = pCurrentNode->children()[numChildren - i - 1];
			if (pChild->marked())
				continue;
			nodeStack.push(pChild);
		}
...
*/
template<typename V, typename P>
void BFS_q(GraphProcessing::Sptr<V> pNode, P p)
{
	std::queue<GraphProcessing::Sptr<V>> nodeQueue;
	nodeQueue.push(pNode);
	while (nodeQueue.size() > 0)
	{

		GraphProcessing::Sptr<V> pCurrentNode = nodeQueue.front();
		nodeQueue.pop();
		pCurrentNode->mark();
		p(pCurrentNode);
		for (auto pChild : pCurrentNode->children())
