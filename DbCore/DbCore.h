...
namespace NoSqlDb
{
  ...
  */
  ...
  template<typename P>
  void showRec(const Key& key, const DbElement<P>& el, std::ostream& out = std::cout)
  {
	  out << "\n  ";
	  out << std::setw(15) << std::left << key.substr(0, 13);
	  out << std::setw(15) << std::left << el.name().substr(0, 13);
	  out << std::setw(26) << std::left << std::string(el.dateTime());
	  out << std::setw(20) << std::left << el.descrip().substr(0, 18);
	  out << std::setw(20) << std::left << "Status: " << std::string(el.payLoad().status()).substr(0, 18);
	  Children children = el.children();
	  if (children.size() > 0)
	  {
		  out << "\n    child keys: ";
  ...
  template<typename P>
  void showStatus(const DbCore<P>& db, std::ostream& out = std::cout)
  {
	  showHeader(true, out);
	  typename DbCore<P>::DbStore dbs = db.dbStore();
	  for (auto item : dbs)
	  {
		  showRec(item.first, item.second, out);
	  }
  }
}
